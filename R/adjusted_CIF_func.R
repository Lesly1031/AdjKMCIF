...
adjusted_CIF <- function(data, time, status, group, covlist, event_code = 1, stratified = "Yes", reference_group = "G&B") {
  ...
  data = data.frame(na.omit(data[,c(time,status,group,covlist)],check.names = F)) # exclude missingness on 2/28/2026
